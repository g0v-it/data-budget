...
SELECT  ?actionId ?action_name ?amount ?last_amount ?missione ?ministero 
WHERE {
  {
    SELECT ?actionId_history ?last_amount 
    WHERE {
      ?s2 a :ActionRecord;
            :actionId ?actionId_history .
            ?s2 :year ?year .
            ?s2 :amount ?last_amount .
  ...
  ?s a :Azione.
  OPTIONAL { ?s :actionId ?actionId}
  OPTIONAL { ?s :actionName ?action_name}
  OPTIONAL { ?s :amount ?amount }
  OPTIONAL { ?s :amount ?last_amount }
  OPTIONAL { ?s :missione ?missione}
  OPTIONAL { ?s :ministero ?ministero}
        
  FILTER (?actionId = ?actionId_history)
}
`
}
