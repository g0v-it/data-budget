...
The following axioms extend the fr-ap :

- the financial report hierarchy maps exactly the financial report taxonomy:`(x a fr:StructuralComponent) ⇒ (x a skos:Concept), (x fr:concept x)` ;
...
- concepts in different mef:FinanctialReport use the same skos:notation `(x skos:notation k),(y skos:notation k) ⇒ (x skos:CloseConcept y)`

the following axioms should be used to derive a Bubble Graph Ontology:

- the source of the BGO Domain is the newest *MEF Financial Report* known (i.e. present in the knowledge graph);
- the source of a bgo:Account must be an *mef:Azione* (i.e. the fourth level of a MEF financial reports);
- *capitolo di spesa* is a breakdown of a bgo:Account;
- account history is inferred taking into account similar components concepts (i.e. skos:closeConcept);
...
- in bgo editorial properties, these rules should be used:
    ...
    - the bgo:description can be  derived from skos:notation
    - the bgo:title for an account can be derived from skos:definition 
...
In the provided examples the following namespaces are assumed:
...
A g0v fr-ap-mef reasoner also should able to create a BGO from financial report information :
...
:account_1 a bgo:Account ;
	...
	bgo:description "Azione 1.1.1.1"@it ;
    ...
    bgo:hasHistoryRec :history_rec_1 ;
    bgo:referenceAmount 200000000000.00 ;
.

:history_rec_1 a bgo:HistoryRec  ;
	dct:source 2018:azione_1;
	bgo:versionLabel "L2018" ; 
